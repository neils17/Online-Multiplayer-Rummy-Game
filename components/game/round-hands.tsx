...
export function RoundHands({
  ...
  return (
    <div className="review-variants">
      {game.players.flatMap((_, viewPlayer) =>
        [false, true].map((showOptimal) => {
          const active =
            viewPlayer === playerIndex &&
            showOptimal === (optimalViews[playerIndex] || false);
          const layout =
            round?.actualLayouts?.[viewPlayer] ||
            game.players[viewPlayer]?.layout;
          const actualGroups = layout?.groups ||
            (round?.declaredGroups?.player === viewPlayer
              ? round.declaredGroups.groups
              : null) || [game.players[viewPlayer].hand.map((c) => c.id)];
          return (
            <div
              className="round-review"
              key={`${viewPlayer}-${showOptimal}`}
              aria-hidden={!active}
              inert={!active}
            ...
            >
              <div className="round-verdict">
                ...
                <div>
                  ...
                  <p>
                    Both hands revealed · Wild cards marked W · Dropped jokers
                    marked Was Joker
                  </p>
                  {round?.multiplier === 2 && (
              ...
              {game.players.map((p, i) => {
                ...
                const infos = groups.map((cards) =>
                  describeGroup(cards, game.wild.r),
                );
